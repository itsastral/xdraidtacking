...
function module:OnInitialize()
	...
	self:RegisterComm("oRA", "OnExtCommReceived")
	self:RegisterComm("CTRA", "OnExtCommReceived")
...
local function commDispatch(sender, isNative, continue, commType, ...)
	...
	addon:debug(("`commDispatch' Sender: %s, Native: %s, Comm type: %s, Message: %s"):format(sender, tostring(isNative), tostring(commType), (...) and tostring(...) or "nil"))
...
function module:OnExtCommReceived(prefix, message, distribution, sender)
	...
	if (prefix == "oRA" or prefix == "CTRA") then
		-- oRA2 sends strings split by spaces.
		-- Legacy CTRA system, ex: CD 1 20 (Rebirth, 20 minute cooldown)
		local cd, spellid, timeleft = unpack(addon:Explode(" ", message))
		if (cd ~= "CD") then
			addon:debug("`OnExtCommReceived'"..prefix.." Comm Received: "..message)
			return
		end
		spellid = ORA_cdtbl[tonumber(spellid)]
		timeleft = timeleft*60
		commDispatch(sender, nil, true, "Cooldown", spellid, timeleft)
	end
...
function module:CooldownCommHandler(event, sender, isNative, spellid, timeleft)
	...
	addon:debug(("`CooldownCommHandler' Event: %s, sender: %s, native: %s, spellid: %s, timeleft: %s."):format(event,sender,tostring(isNative),tostring(spellid),tostring(timeleft)))
	...
	if bar then
		...
		if not spellicon then spellicon = select(10,GetItemInfo(spellid)) end
...
function module:UpdateCommHandler(event, sender, isNative, version, spells)
	...
	addon:debug("`UpdateCommHandler' Sender: "..sender.." Version: "..version)
