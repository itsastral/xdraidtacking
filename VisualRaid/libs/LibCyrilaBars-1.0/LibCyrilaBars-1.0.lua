...
local function OnUpdate(self,elapsed)
	...
	while bar do
		if not bar.data.endTime then
			...
			if bar.data.timeleft>=0 then bar.timer:SetText(MMSS(bar.data.timeleft)) end
			...
			if(bar.data.flash) then
				...
				--a = (cos(self.fval^fvalN)+1)*.5
				local a = 0.5 * sin(sqrt(bar.data.timeleft)*1500) + 0.5
				bar.statusbarbg:SetAlpha(a);
			end

			if(bar.data.expiring) then
				--bar.bg:SetBackdropColor(1,1,1,1);
				bar.timer:SetText("")
				local animlength = 1
				local startcollapse = .9
				local startfade = .85

				if not bar.data.expiretimestart then
					bar.data.expiretimestart = GetTime()
					bar.data.expiretimecollapse = GetTime()+startcollapse
					bar.data.expiretimefade = GetTime()+startfade
					bar.data.expireinitheight1 = bar:GetHeight()
					bar.data.expireinitheight2 = bar.icon:GetHeight()
					bar.data.expireinitheight3 = bar.statusbar:GetHeight()
				end
				...
				if GetTime()>=bar.data.expiretimecollapse then
					local pcpl = (GetTime()-bar.data.expiretimecollapse) / (animlength-startcollapse)
					bar:SetHeight(bar.data.expireinitheight1 - bar.data.expireinitheight1*pcpl)
					bar.icon:SetHeight(bar.data.expireinitheight2 - bar.data.expireinitheight2*pcpl)
					bar.statusbar:SetHeight(bar.data.expireinitheight3 - bar.data.expireinitheight3*pcpl)
					if pcpl>=1 then
						pcpl = 1
						bar.bg2:Hide()
						bar.data.expired = true
					end
				end

				if GetTime()>=bar.data.expiretimefade then
					local pcpl = (GetTime()-bar.data.expiretimefade) / (animlength-startfade)
					bar:SetAlpha(1 - 1*pcpl)
					if pcpl>=1 then
						pcpl = 1
						bar:Hide()
						bar.bg2:Hide()
				...
				if bar.data.expired then
					bar:Release()
				end
			end
			if(bar.data.cancelexpiring) then
				if bar.data.expiretimestart then
					bar:SetHeight(bar.data.expireinitheight1)
					bar.icon:SetHeight(bar.data.expireinitheight2)
					bar.statusbar:SetHeight(bar.data.expireinitheight3)
					bar:SetAlpha(1)
					bar.data.expiretimestart = nil
				end
				bar.bg2:Hide()
				bar.statusbarbg:SetVertexColor(.1,.1,.1)
				bar.statusbarbg:SetAlpha(1)
				bar.data.cancelexpiring = nil
			end
...
function prototype:Release()
	if self.data.AnimateExpiration and not self.data.expired then
		self:EffectExpire()
		return
	end
...
function prototype:EffectExpire()
	self.data.expiring = true
end
...
function prototype:SetTimeleft(timeleft)
	...
	self.data.flash = nil
	if self.data.expiring then self.data.cancelexpiring = true end
	self.data.expiring = nil
	...
	self:CancelAllTimers()
	self:ScheduleTimer("EffectFlash", timeleft-3)
...
local function ApplySettings(bar, t)
	...
	if not t.SparkEnabled or t.SparkEnabled==0 then bar.spark:Hide() end
	...
	bar.data.AnimateExpiration = (t.AnimateExpiration==1) and true or nil
...
local function NewBarPrototype()
	...
	bg2:SetPoint("TOPLEFT", statusbar, "TOPLEFT", -21, 4)
	bg2:SetPoint("BOTTOMRIGHT", statusbar, "BOTTOMRIGHT", 3, -3)
	...
	bg2:SetBackdropColor(1, 1, 1, 1)
	bg2:SetBackdropBorderColor(1, 1, 1)
	bg2:Hide()
	self.bg2 = bg2
	...
	icon:SetPoint("BOTTOM",self,"BOTTOMLEFT", -9, 0);
	icon:SetSize(15, 15);
	icon:Show();
	...
	iconbg:SetBackdropColor(0, 0, 0, 1)
	iconbg:Show()
	self.iconbg = iconbg
...
function lib:SpawnHeader(name, xOfs, yOfs, point, rpoint, bartable, sortmethod, label)
	...
	insets = {left = 0,right = 0,top = 0,bottom = 0}});
	header:SetBackdropColor(.04, .05, .06, 1)
	...
	bg:SetPoint("BOTTOMRIGHT", header, "BOTTOMRIGHT", 1, -1)
	bg:SetFrameStrata("LOW")
	bg:SetBackdrop({bgFile = "Interface\\Buttons\\WHITE8X8",
	insets = {left = 0,right = 0,top = 0,bottom = 0}});
	bg:SetBackdropColor(0.4, 0.6, 0.8, 1)
	--bg:Show()
	header.bg = bg

	local fs = header:CreateFontString(nil, "OVERLAY")
	fs:SetPoint("LEFT", header, 4, 0)
	fs:SetFont(bartable.LabelFont, bartable.LabelFontSize, bartable.LabelFontFlags)
	fs:SetShadowOffset(0, 0)
	fs:SetTextColor(1, 1, 1)
	fs:SetText(header.label);
	header.fs = label
	
	header.tr = header:CreateTitleRegion()
	header.tr:SetAllPoints(header)
	
	header:SetHeight(bartable.Height)
	header:SetWidth(bartable.Width)
